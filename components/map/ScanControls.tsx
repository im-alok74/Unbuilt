...
import { Radar, MapPin, AlertTriangle, Check, ChevronDown, ChevronUp } from "lucide-react";
...
export function ScanControls({ onScanned }: { onScanned: () => void }) {
  ...
  React.useEffect(() => {
    try {
      ...
      setMinimized(localStorage.getItem("unbuilt.scanPanelMinimized") === "1");
    } catch {}
  }, []);
  ...
  function toggleMinimized() {
    setMinimized((m) => {
      const next = !m;
      try {
        localStorage.setItem("unbuilt.scanPanelMinimized", next ? "1" : "0");
      } catch {}
      return next;
    });
  }
